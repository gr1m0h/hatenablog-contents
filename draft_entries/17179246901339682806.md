...
APIやWebアプリケーションのパフォーマンス評価には負荷テストが欠かせません。インフラのキャパシティプランニングにおいても同様です。
...
k6は、JavaScriptやTypeScriptでテストスクリプトを記述できる負荷テストツールです。
...
また、asdf-vmやmiseを使用してインストールすることもできます。
...
ちなみにasdf-vmやmiseがk6インストール時にbackendで利用しているk6のasdf-pluginは私が作成したものです。

[https://github.com/gr1m0h/asdf-k6:embed:cite]

## テストスクリプトの作成

`k6 new script.ts`でテンプレートを生成できます。k6 1.0からTypeScriptがネイティブサポートされ、トランスパイル不要で直接実行できます。
...
閾値を満たさない場合、k6は0以外の終了コードで終了します。
CI/CDパイプラインに組み込む際に便利です。
...
// 1. init: Executed once per VU at the start
const BASE_URL = "https://jsonplaceholder.typicode.com";

// 2. setup: Executed once at the start of the entire test
...
// 3. default: Executed repeatedly by each VU (main test logic)
export default function (data) {
  http.get(`${BASE_URL}/posts/${data.postId}`);
}

// 4. teardown: Executed once at the end of the entire test
...
`__ENV`オブジェクトで環境変数にアクセスできます。環境ごとにURLやパラメータを切り替える際に便利です。
...
```bash
BASE_URL=https://staging.example.com API_KEY=xxx k6 run script.ts
# Or specify with -e option
k6 run -e BASE_URL=https://staging.example.com script.ts
```

## カスタムメトリクス

組み込みメトリクス以外に、独自のメトリクスを定義できます。
